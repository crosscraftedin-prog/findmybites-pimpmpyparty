...
function transformVendor(v: any): Vendor {
  return {
    ...
    languagesSpoken: v.languagesSpoken ?? null,
    yearStarted: v.yearStarted ?? null,
    businessRegNumber: v.businessRegNumber ?? null,
    gstVatNumber: v.gstVatNumber ?? null,
    hideAddress: v.hideAddress ?? false,
    holidayMode: v.holidayMode ?? false,
    vacationMode: v.vacationMode ?? false,
    emergencyClosure: v.emergencyClosure ?? false,
    homeService: v.homeService ?? false,
    onlineConsultation: v.onlineConsultation ?? false,
    maxOrder: v.maxOrder ?? null,
    prepTime: v.prepTime ?? null,
    bookingNotice: v.bookingNotice ?? null,
    responseRate: v.responseRate ?? 0,
    profileViews: v.profileViews ?? 0,
    productViews: v.productViews ?? 0,
    galleryViews: v.galleryViews ?? 0,
