    ...
    <div className="space-y-4">
      ...
      <div>
        ...
        <p className="mb-2 text-xs text-muted-foreground">
          Drag & drop or click to upload. Drag to reorder.
        </p>
        <GalleryUpload
          images={form.images || []}
        ...
        <>
          ...
          <div className="min-w-0 flex-1 overflow-y-auto p-4 sm:p-6">
            <div className="mx-auto max-w-2xl space-y-4">
              ...
              {/* CARD 1: Quick Publish (always expanded — minimal fields only) */}
              <div className="space-y-4 rounded-xl border border-border bg-card p-4">
                ...
                <div className="rounded-lg border border-border bg-muted/30 p-3">
                  <div className="mb-2 flex items-center justify-between">
                    <span className="text-xs font-semibold">Product Completion</span>
                    <span className="text-xs font-bold text-amber-600">{qualityScore}%</span>
                  </div>
                  <div className="h-1.5 overflow-hidden rounded-full bg-muted">
                    <div className={cn("h-full rounded-full transition-all", qualityScore >= 80 ? "bg-emerald-500" : qualityScore >= 50 ? "bg-amber-500" : "bg-red-500")} style={{ width: `${qualityScore}%` }} />
                  </div>
                  {/* Missing items checklist */}
                  <div className="mt-3 space-y-1">
                    {validation.checks.filter(c => !c.passed && !c.optional).map(c => (
                    ...
                    {validation.checks.filter(c => !c.passed && !c.optional).length === 0 && (
                      <p className="text-[11px] font-medium text-emerald-600">✓ All required fields complete!</p>
                    )}
                  </div>
                </div>
              </div>

              {/* CARD 2: Product Details (collapsed — includes tags, dietary, allergens, occasion, template fields) */}
              <ExpandableCard title="Product Details" icon={Info} badge={qualityScore > 0 ? undefined : undefined}>
                {/* Quick Tags — moved here from Card 1 */}
                <div className="mb-4 space-y-4">
                  <div>
                    <h3 className="text-base font-bold">Tags & Attributes</h3>
                  ...
                  <div>
                    ...
                    <div className="mt-2 flex flex-wrap gap-2">
                      {DIETARY_OPTIONS.map(opt => (
                        <ChipToggle key={opt.key} label={opt.label} active={!!form[opt.key]} onClick={() => set(opt.key, !form[opt.key])} />
                      ))}
                    </div>
                  </div>
                  {/* Allergens */}
                  <div>
                    ...
                    <div className="mt-2 flex flex-wrap gap-2">
                      {ALLERGEN_OPTIONS.map(a => (
                        <ChipToggle key={a} label={a} active={(productInfo.allergens ?? []).includes(a)}
                          onClick={() => {
                            const current = productInfo.allergens ?? [];
                            setProductInfo({ ...productInfo, allergens: current.includes(a) ? current.filter(x => x !== a) : [...current, a] });
                          }}
                        />
                      ))}
                    </div>
                  ...
                  <div>
                    ...
                    <div className="mt-2 flex flex-wrap gap-2">
                      {OCCASION_OPTIONS.map(o => (
                        <ChipToggle key={o} label={o} active={(productInfo.occasionTags ?? []).includes(o)}
                          onClick={() => {
                            const current = productInfo.occasionTags ?? [];
                            setProductInfo({ ...productInfo, occasionTags: current.includes(o) ? current.filter(x => x !== o) : [...current, o] });
                          }}
                        />
                      ))}
                    </div>
              ...
              {/* CARD 3: Variants & Customization (collapsed — empty state when no variants) */}
              <ExpandableCard title="Variants & Customization" icon={Package} badge={(form.variants?.length ?? 0) > 0 ? `${form.variants.length}` : undefined}>
                {(!form.variants || form.variants.length === 0) && (
                  <div className="mb-4 rounded-lg border border-dashed border-border bg-muted/20 p-6 text-center">
                    ...
                    <p className="mt-2 text-sm font-medium text-muted-foreground">No variants yet</p>
                    <p className="mt-0.5 text-xs text-muted-foreground">Add sizes, flavours, or packages with their own pricing.</p>
                    <Button variant="outline" size="sm" className="mt-3 gap-1" onClick={() => set("variants", [...(form.variants || []), { type: "", name: "", price: "", offerPrice: "", description: "", available: true }])}>
                      <Plus className="size-3.5" /> Add Variant
                    </Button>
                  </div>
                )}
                {(form.variants?.length ?? 0) > 0 && renderVariantCards()}
                <div className="mt-4">
                  {renderFields(["customisation"])}
                </div>
              </ExpandableCard>

              {/* CARD 4: Delivery (collapsed — simplified, advanced fields on demand) */}
              <ExpandableCard title="Delivery" icon={Truck}>
                {/* Simple delivery options first */}
                <div className="space-y-3">
                ...
                {/* Advanced: Inventory, Scheduling, Lead Time */}
                <div className="mt-4 border-t border-border pt-4">
                  {renderInventory()}
                </div>
              </ExpandableCard>

              {/* CARD 5: Marketing (collapsed — empty state when no SEO) */}
              <ExpandableCard title="Marketing" icon={Search} badge={form.metaTitle ? undefined : undefined}>
                {(!form.metaTitle && !form.metaDescription) && (
                  <div className="mb-4 rounded-lg border border-dashed border-border bg-muted/20 p-6 text-center">
                    <Search className="mx-auto size-8 text-muted-foreground/40" />
                    <p className="mt-2 text-sm font-medium text-muted-foreground">Improve your Google ranking</p>
                    <p className="mt-0.5 text-xs text-muted-foreground">Add SEO title, description, and keywords to appear in search results.</p>
                  </div>
                )}
                {renderSEO()}
                {/* Featured toggle */}
          ...
          {/* Sticky bottom bar with Completion % + Save Draft + Publish + Preview */}
          <div className="shrink-0 border-t border-border bg-card px-4 py-3 [padding-bottom:calc(env(safe-area-inset-bottom)+0.75rem)]">
            <div className="mx-auto flex max-w-2xl items-center justify-between gap-2">
              ...
              <div className="flex items-center gap-2">
                <Button variant="outline" onClick={() => setShowPreview(true)} size="sm" className="gap-1">
                  <Eye className="size-3.5" /> Preview
                </Button>
                <Button onClick={handlePublish} disabled={saving || published}
                  className="gap-1.5 bg-emerald-600 text-white hover:bg-emerald-700" size="sm">
          ...
          {/* Floating Josh AI Assistant button */}
          <button
            onClick={() => { if (form.name?.trim()) { setHasGeneratedAI(true); generateWithAI(); } else { toast.error("Enter a product name first"); } }}
            disabled={aiGenerating || !form.name}
            className="fixed bottom-20 right-4 z-40 flex items-center gap-1.5 rounded-full bg-purple-600 text-white px-4 py-2 shadow-lg hover:bg-purple-700 disabled:opacity-50"
